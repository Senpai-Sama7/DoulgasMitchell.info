...
export const JSXPreviewContent = memo(
  ({ className, ...props }: JSXPreviewContentProps) => {
    ...
    const [streamingErrorKey, setStreamingErrorKey] = useState<string | null>(
      null
    );
    ...
    const handleError = useCallback(
      (err: Error) => {
        ...
        if (isStreaming) {
          setStreamingErrorKey(processedJsx);
          return;
        }
    ...
    const displayJsx =
      isStreaming && streamingErrorKey === processedJsx
        ? lastGoodJsx || processedJsx
        : processedJsx;
